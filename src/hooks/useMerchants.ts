...
export const useMerchants = (categoryIds?: string[], searchTerm?: string, startTime?: string, endTime?: string, location?: string, bounds?: { north: number; south: number; east: number; west: number }, radiusMiles?: number, showOffersOnly?: boolean, selectedDays?: number[], gpsCoordinates?: { lat: number; lng: number }, carouselId?: string) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Apply radius filtering if specified (must have location or GPS coordinates)
      let filteredData = data;
      if (radiusMiles && (location || gpsCoordinates)) {
        console.log('Applying radius filtering:', radiusMiles, 'miles from', location || 'GPS location');
        ...
        try {
          ...
          if (gpsCoordinates) {
            ...
            console.log('Using GPS coordinates for radius filtering:', locationData);
          } else {
