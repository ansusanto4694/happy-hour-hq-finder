...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: orphanSessions, error: orphanError } = await supabase
      .rpc('get_orphan_sessions');

    if (!orphanError && orphanSessions) {
      console.log(`[Backfill] Found ${orphanSessions.length} orphan sessions to insert`);
    }

    // Get sessions that don't exist in user_sessions but have events
    const { data: missingSessionIds } = await supabase
      .from('user_events')
      .select('session_id')
      .not('session_id', 'in', `(SELECT session_id FROM user_sessions)`);

    const uniqueMissingIds = [...new Set(missingSessionIds?.map(s => s.session_id) || [])];
    console.log(`[Backfill] Found ${uniqueMissingIds.length} missing sessions`);
