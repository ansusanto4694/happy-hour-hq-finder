...
BEGIN
  ...
  IF (TG_OP = 'INSERT') OR 
    ...
    -- Log the response
    RAISE LOG 'HTTP request sent for merchant ID %, response ID: %', NEW.id, response_id;
...
-- Also create a function to manually trigger geocoding for existing records
...
BEGIN
  ...
  RAISE LOG 'Manual geocoding triggered for merchant ID %, response ID: %', merchant_id, response_id;
END;
$$ LANGUAGE plpgsql;
